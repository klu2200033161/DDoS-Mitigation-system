...
INTERFACE_NAME = None  # auto detect
FEATURE_WINDOW = 5
...
# ---------------- packet capture ----------------
def packet_callback(packet):
    global LIVE_PACKET_COUNT, LIVE_BYTE_COUNT

    if not MONITORING_ACTIVE or IP not in packet:
        return
...
def realtime_rate_calculator():
    ...
    while RUNNING:
        ...
        with LOCK:
            pps = LIVE_PACKET_COUNT
            bps = LIVE_BYTE_COUNT
            LIVE_PACKET_COUNT = 0
            LIVE_BYTE_COUNT = 0

        LATEST_ANALYSIS["timestamp"] = int(time.time())
        LATEST_ANALYSIS["total_packets"] = pps
        LATEST_ANALYSIS["inbound_rate"] = round(bps, 2)


# ---------------- detection ----------------
def extract_and_classify(packets):
    ...
    for ip,data in flows.items():
        ...
        bps=data["bytes"]/duration
        risk=min(100,int(pps*1.5))

        # dominant protocol
        proto=max(("TCP",data["tcp"]),("UDP",data["udp"]),("ICMP",data["icmp"]),key=lambda x:x[1])[0]

        # ----- DDoS TYPE DETECTION -----
        ddos_type="Normal Traffic"

        if risk>=90:

            # SYN flood
            if data["syn"]>data["ack"]*3 and data["syn"]>50:
                ddos_type="TCP SYN Flood"

            # UDP flood
            elif data["udp"]>data["tcp"]*2 and pps>200:
                ddos_type="UDP Flood"

            # ICMP flood
            elif data["icmp"]>data["tcp"]+data["udp"] and pps>150:
                ddos_type="ICMP Flood"

            # Reset attack
            elif data["rst"]>data["ack"]*2:
                ddos_type="TCP Reset Attack"

            # Amplification
            elif bps>2000000 and pps<200:
                ddos_type="Reflection/Amplification Attack"

            else:
                ddos_type="Mixed Flood Attack"

        elif risk>=50:
            ddos_type="Suspicious Behaviour"

        status="Normal"
        if risk>=90: status="DDoS Attack"
        elif risk>=50: status="Monitored"
...
def start_analysis_loop():
    while RUNNING:
        ...
        LATEST_ANALYSIS["high_risk_ips"]=extract_and_classify(packets)
